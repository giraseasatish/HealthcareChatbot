...
df = pd.read_csv(r"C:\Users\Admin\Downloads\Updated Code\datasets.csv",encoding="ISO-8859-1")
...
import duckduckgo_search as ddg  # Assuming you're using duckduckgo_search

def getDiseaseInfo(keywords):
    """
    Fetches disease information using DuckDuckGo search and handles potential errors.

    Args:
        keywords (list): A list of keywords to search for (e.g., ["cough", "fever", "sore throat"]).

    Returns:
        str: The body text of the first relevant search result (if found),
    ...
    try:
        ...
        if results:  # Check if results is not None
            ...
            return "No information found about this disease on DuckDuckGo."
    except Exception as e:  # Catch any unexpected exceptions
        return f"An error occurred while searching for disease information: {e}"
